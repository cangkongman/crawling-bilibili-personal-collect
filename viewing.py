...
def viewing(path_collect_data):
    ...
    for i in dir_names:
        ...
        with open(i+'.json', 'r', encoding='utf-8') as fp:
            file = json.load(fp)
        num = 1
        sht.write_merge(0, 0, 0, 1, 'Title', headStyle())
        sht.write(0, 2, 'UP name', headStyle())
        sht.write(0, 3, 'UP UID', headStyle())
        sht.write(0, 4, 'play', headStyle())
        sht.write(0, 5, 'collect', headStyle())
        sht.write(0, 6, 'danmaku', headStyle())
        ...
        for j in file:
            sum += 1
            sht.write_merge(num, num+2, 0, 1, j['title'], TitleStyle())
            ...
            sht.write_merge(num+1, num+2, 2, 6, j['intro'], introStyle())
            if(sum < len(file)):
